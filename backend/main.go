...
func main() {
	...
	mux.HandleFunc("GET /api/apps/{audience}", appGetHandler(authStore))
	mux.HandleFunc("POST /api/admin/apps", adminAppsMirrorHandler(adminSessions, authStore))
	mux.HandleFunc("POST /api/admin/apps/{audience}/api-key", adminAppAPIKeyIssueHandler(adminSessions, authStore))
	awardStore := NewAwardStore(db)
	mux.HandleFunc("POST /api/awards", awardsCreateHandler(awardStore, authStore))
	mux.HandleFunc("GET /api/profiles/{address}/achievements", achievementsListHandler(awardStore, authStore))
