...
describe('createProfileClient', () => {
    it('uses the default base URL when options omit baseUrl', async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith(`${DEFAULT_BASE_URL}/api/profile/NQ01TEST`, expect.any(Object));
    });
});
describe('getProfileByAddress', () => {
    it('returns parsed profile on 200', async () => {
        ...
        expect(fetch).toHaveBeenCalledWith('https://nc.example/api/profile/NQ01TEST', expect.any(Object));
    });
...
describe('resolveHandle', () => {
    it('returns parsed claim on 200', async () => {
        ...
        expect(fetch).toHaveBeenCalledWith('https://nc.example/api/resolve/ada', expect.any(Object));
    });
    it('returns null on 404', async () => {
        vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ ok: false, status: 404 }));
        const client = createProfileClient({ baseUrl: 'https://nc.example' });
...
describe('resolveHandleForPayment', () => {
    it('uses the fresh, non-cacheable payment resolver and parses the claim', async () => {
        ...
        expect(fetch).toHaveBeenCalledWith('https://nc.example/api/pay/resolve/ada', {
    ...
    it('fails closed when fresh payment resolution is unavailable', async () => {
        ...
        const client = createProfileClient({ baseUrl: 'https://nc.example' });
        await expect(client.resolveHandleForPayment('ada')).rejects.toThrow('payment handle resolve failed: 503');
    });
});
describe('getHandleByAddress', () => {
    it('returns parsed claim on 200', async () => {
        ...
        expect(fetch).toHaveBeenCalledWith('https://nc.example/api/handles/by-address/NQ01TEST', expect.any(Object));
    });
    it('returns null on 404', async () => {
        vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ ok: false, status: 404 }));
        const client = createProfileClient({ baseUrl: 'https://nc.example' });
