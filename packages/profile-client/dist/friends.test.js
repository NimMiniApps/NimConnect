...
describe('friends methods', () => {
    it('listFriends sends the session header', async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith('https://nc.example/api/friends', expect.objectContaining({
    ...
    it('listFriendRequests hits /api/friends/requests', async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith('https://nc.example/api/friends/requests', expect.any(Object));
    });
    it('sendFriendRequest posts { to }', async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith('https://nc.example/api/friends/requests', expect.objectContaining({
            method: 'POST',
    ...
    it('acceptFriendRequest / declineFriendRequest / removeFriend', async () => {
        ...
        expect(fetchMock).toHaveBeenNthCalledWith(1, 'https://nc.example/api/friends/requests/id-1/accept', expect.objectContaining({ method: 'POST' }));
        expect(fetchMock).toHaveBeenNthCalledWith(2, 'https://nc.example/api/friends/requests/id-2/decline', expect.objectContaining({ method: 'POST' }));
        expect(fetchMock).toHaveBeenNthCalledWith(3, 'https://nc.example/api/friends/NQ26TEST', expect.objectContaining({ method: 'DELETE' }));
    });
