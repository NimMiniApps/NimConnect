...
import { fetchHandleRegistry, DEFAULT_RPC_URL, IncompleteRegistryHistoryError, AmbiguousRegistryOrderingError, } from './rpc.js';
afterEach(() => {
    vi.unstubAllGlobals();
});
function claimTxHex(handle) {
...
describe('fetchHandleRegistry', () => {
    ...
    it('resolves HTLC-routed claims via a live account lookup', async () => {
        const fetchMock = mockRpc({
            getTransactionsByAddress: ([address]) => address === HANDLE_REGISTRY_ADDRESS
                ? [
                    {
                        hash: 't1',
    ...
    it('falls back to creation-tx data when the HTLC account has no sender field', async () => {
        ...
        const creationData = '91c5d65cbf079159b61d72bfca4ff1f5fd063227a70b9e44a448b5183ac4e186cd749d3d889fff840100000000000000000000000000000000000000';
        const fetchMock = mockRpc({
            getTransactionsByAddress: ([address]) => address === HANDLE_REGISTRY_ADDRESS
                ? [
                    {
    ...
    it('surfaces RPC errors', async () => {
        vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ ok: true, json: async () => ({ jsonrpc: '2.0', id: 1, error: { message: 'boom' } }) }));
