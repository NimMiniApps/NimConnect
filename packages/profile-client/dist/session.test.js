...
describe('userSessionChallenge', () => {
    it('matches the backend v2 challenge format', () => {
        expect(userSessionChallenge('NQ17 VERV F3MQ 283T NRSR FPJG 55BJ PMHC N8MD', 1700000000, 'nimworld')).toBe('nimconnect-session:v2:NQ17VERVF3MQ283TNRSRFPJG55BJPMHCN8MD:1700000000:nimworld');
    });
});
describe('userSessionChallengeV1', () => {
    it('matches the backend v1 challenge format', () => {
        expect(userSessionChallengeV1('NQ17 VERV F3MQ 283T NRSR FPJG 55BJ PMHC N8MD', 1700000000)).toBe('nimconnect-session:v1:NQ17VERVF3MQ283TNRSRFPJG55BJPMHCN8MD:1700000000');
    });
});
...
describe('createSession', () => {
    it('signs the v1 challenge when no audience is configured', async () => {
        ...
        expect(signMessage).toHaveBeenCalledWith('nimconnect-session:v1:NQ17VERVF3MQ283TNRSRFPJG55BJPMHCN8MD:1700000000');
        expect(fetchMock).toHaveBeenCalledWith('https://nc.example/api/session', expect.objectContaining({
            method: 'POST',
            body: JSON.stringify({
                address: 'NQ17 VERV F3MQ 283T NRSR FPJG 55BJ PMHC N8MD',
                publicKey: 'aa',
    ...
    it('signs the v2 challenge and posts audience when configured', async () => {
        ...
        expect(signMessage).toHaveBeenCalledWith('nimconnect-session:v2:NQ17VERVF3MQ283TNRSRFPJG55BJPMHCN8MD:1700000000:nimworld');
        expect(fetchMock).toHaveBeenCalledWith('https://nc.example/api/session', expect.objectContaining({
            method: 'POST',
            body: JSON.stringify({
                address: 'NQ17 VERV F3MQ 283T NRSR FPJG 55BJ PMHC N8MD',
