...
function isHexDigitsString(s: string): boolean {
 ...
 */
export function ownerFromHtlcCreationData(dataHex: string): string | null {
  const raw = hexToBytes(dataHex)
  if (!raw || raw.length < 40) return null
  return userFriendlyAddress(raw.slice(0, 20))
}

async function claimantAddress(
  tx: RegistryTx,
  resolveHtlcOwner: ((contractAddress: string) => string | null | Promise<string | null>) | undefined,
  cache: Map<string, string | null>,
): Promise<string> {
  if (tx.fromType !== HTLC_ACCOUNT_TYPE) return tx.sender
...
export function resolveHandleByAddress(
  registry: Map<string, ResolvedHandleClaim>,
  address: string,
): ResolvedHandleClaim | null {
  let best: ResolvedHandleClaim | null = null
  for (const claim of registry.values()) {
    if (compactAddress(claim.address) !== compactAddress(address)) continue
    if (
      !best ||
...
export function isHandleAvailable(registry: Map<string, ResolvedHandleClaim>, handle: string): boolean {
