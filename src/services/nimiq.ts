...
/** True when Nimiq Pay injected host context before this page loaded. */
export function isInsideNimiqPay(): boolean {
  return typeof window !== 'undefined' && (window.nimiqPay != null || window.nimiq != null)
}

/** Static for the page session — safe to use in templates. */
export const insideNimiqPay = isInsideNimiqPay()
...
export const walletStatus = ref<'idle' | 'connecting' | 'ready' | 'unavailable'>(
  insideNimiqPay ? 'idle' : 'unavailable',
)
...
export function getProvider(): Promise<NimiqProvider | null> {
  if (!isInsideNimiqPay()) return Promise.resolve(null)
...
export function connectWallet(): Promise<string | null> {
  if (!isInsideNimiqPay()) {
    ...
    addressPromise ??= Promise.resolve(null)
